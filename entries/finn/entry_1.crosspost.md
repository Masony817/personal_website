<!--
Cross-post copy for X Articles / Substack.
Canonical version: https://masonyarbrough.com/blog/finn-entry-1
Set the canonical URL to that link in Substack (Settings > SEO) so the site keeps the search credit.
Image URLs below are absolute and live, so they can be pasted or re-uploaded directly.
-->
...
![The whole robot, so far. Two hoverboard hub motors, a 4040 base, a 100 cm mast, and a wooden gantry to catch it.](https://masonyarbrough.com/images/finn-entry1/web/hero-robot.webp)

*The whole robot, so far. Nothing above the axle exists yet except the space for it.*
...
![A hoverboard opened up, showing the hub motor and factory controller.](https://masonyarbrough.com/images/finn-entry1/web/hoverboard.webp)

*The donor. Everything in this photo except the motor got thrown away.*
...
[Bracketbot](https://www.bracket.bot/) has been doing good work in this form factor, and their cheap-and-capable thing is a lot of why I stopped considering legs and what inspired a bit of the form factor I went with.

## The frame

4040 aluminum extrusion. Roughly 50 cm across the base, 100 cm mast for now.

![The robot base on a cutting mat, before the mast went on.](https://masonyarbrough.com/images/finn-entry1/web/base-bench.webp)

*The base before the mast went on. Printed axle blocks, one drive per wheel, and lever connectors doing power distribution in the middle.*
...
Each hub motor runs off a [Moteus r4.11](https://mjbots.com/products/moteus-r4-11) from mjbots. Shout out to Josh Pieper for that whole ecosystem.

![Top-down view of the base showing both hub motors, both Moteus boards, and the CAN bus between them.](https://masonyarbrough.com/images/finn-entry1/web/base-axle.webp)

*Both Moteus boards on the base rail. The twisted pair running between them is the CAN-FD bus.*
...
![The lower wiring harness as drawn.](https://masonyarbrough.com/images/finn-entry1/web/wiring-diagram.webp)

![The Teensy, IMU, and CAN transceiver wired up loose on a desk.](https://masonyarbrough.com/images/finn-entry1/web/brain-bench.webp)

*The lower harness as drawn, and the same thing as it actually existed the first time it powered up on the desk.*

That breakout is a TJA1051 transceiver, soldered together because I didn't want to wait for the right connectors to show up, and I fully expect that to bite me. CAN-FD rather than classic CAN because Moteus supports the faster data phase and the larger frames, and I want low-latency command and telemetry.
...
![The IMU, Teensy, and CAN transceiver in printed brackets on the robot's base rail.](https://masonyarbrough.com/images/finn-entry1/web/brain-mounted.webp)

*On the robot: IMU, Teensy, and transceiver in printed clips on the base rail, low and close to the wheel axle.*
...
![A 24V Greenworks cordless tool battery on a workbench.](https://masonyarbrough.com/images/finn-entry1/web/battery.webp)

*The whole power system: a cordless tool battery, until I trust myself to build a pack.*
...
The current concept is a two-axis gimbal with two camera modules. This came out of watching my fiancée work with cameras, where small deliberate movements read as intentional almost immediately. Convergence can signal focus. Pan and tilt moving together can show attention. A little asymmetry can suggest curiosity or uncertainty. All of it without a screen displaying cartoon eyes, which I find uncanny, and which the industry has adopted far more widely than I think it has earned.

> The working hypothesis: a small vocabulary of mechanically legible camera movements is enough to convey attention and internal state without a humanlike face, and a policy can learn to extend that vocabulary instead of me hand-authoring every gesture.

The problem is that if the head moves independently, the stereo extrinsics move with it. Getting reliable depth out of that would need precise joint sensing and dynamic calibration, which will be a real sub-project on its own. The fallback is fixed cameras doing perception and the head doing nothing but social signaling. I don't know which way it goes yet. If a chest camera shows up on FINN later, that's what happened.

Arms are less settled. Integrated quasi-direct-drive actuators, like something from RobStride with its planetary reducer and encoder in the housing, are still where I think it ends up. Although, [SO-101](https://github.com/TheRobotStudio/SO-ARM100) arms are cheap enough that they might be the right thing to bolt on for the first round of manipulation and data collection, and I keep going back and forth on whether starting cheap there teaches me more or less, since I'll be tempted to just use the Hugging Face LeRobot library rather than build out the dynamics and libraries on my own.
...
<!-- Video: upload images/finn-entry1/data/balance.gif (or web/balance.mp4) here. -->

![Three plots over twelve seconds: body lean, wheel travel, and per-wheel torque under LQR.](https://masonyarbrough.com/images/finn-entry1/web/balance-chart.webp)

*The identified model under LQR in MuJoCo, shoved three times with 22 N for 120 ms. Worst case is 8.0° of lean and 1.3 s back to upright, with the torque command clipping the 1 N·m actuator limit each time. Note the wheel travel: it does not come back to where it started.*

So that's where it sits. First sysid pass done, LQR balancing the identified model in sim. Balancing while creeping forward, actually. There's a slow drift I haven't run down yet, and I don't know whether it's the cost function not penalizing position, steady state error from something I mismodeled in the friction, or the linearization sliding out from under it as the state moves from upright. The next entry is fixing this, how I identified the model, and however much sim-to-real gap is left over after I go find out.

Everything lives at [github.com/Masony817/finn](https://github.com/Masony817/finn). Firmware, CAD, models, data, and experiment logs, updated as I go.
...
FINN is self-funded. There's a project-support link on my site for anyone who wants to help cover parts and fabrication.

*Originally published at [masonyarbrough.com/blog/finn-entry-1](https://masonyarbrough.com/blog/finn-entry-1).*
